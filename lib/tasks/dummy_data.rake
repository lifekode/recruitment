namespace :dummy_data do
  desc 'create developers'

  task :create_developers, [:num_of_dev, :num_of_language, :num_of_pro_lan] => :environment do |t, arg|
    ActiveRecord::Base.transaction do
      # create languages
      languages = create_languages(arg[:num_of_language].to_i)

      # create programming_languages
      programming_languages = create_programming_languages(arg[:num_of_pro_lan].to_i)

      # create developer
      create_developers(languages, programming_languages, arg[:num_of_dev].to_i)
    end
  end

  def create_languages(num_of_language = 100)
    num_of_language.times do
      code = locales[Faker::Address.country]
      begin
  ...
  def create_programming_languages(num_of_pro_lan = 100)
    num_of_pro_lan.times do
      begin
  ...
  def create_developers(languages, programming_languages, num_of_dev = 100)
    num_of_dev.times do
      developer = FactoryBot.create :developer

      # add language for developer
      (1..10).to_a.sample.times do
        begin
          developer.languages << languages.sample
        rescue Exception => e
          print e
        end
      end

      # add language and program language for developer
      (1..10).to_a.sample.times do
        begin
          developer.skills << programming_languages.sample
        rescue Exception => e
          print e
        end
      end
